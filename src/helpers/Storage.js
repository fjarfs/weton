...
const qzkSharefPreferences = {
	sharedPreferencesName: 'ayosrc',
	keychainService: 'qzk-qw'
};
...
const setAuthorization = async value => {
	try {
		...
		await SInfo.setItem('authorization', encrypt, qzkSharefPreferences);
	} catch (error) {
		console.error('Error Saving Data', error);
	}
};
...
const removeAuthFromStorage = async () => {
	try {
		...
		await SInfo.deleteItem('authorization', qzkSharefPreferences);
...
const setProfileFromRest = async () => {
	try {
		...
		await SInfo.setItem('profile', encrypt, qzkSharefPreferences);
		return profile;
	} catch (err) {
...
const getProfileFromStorage = async () => {
	try {
		const profile = await SInfo.getItem('profile', qzkSharefPreferences);
		if (profile !== null) {
